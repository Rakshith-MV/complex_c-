...
# --- SEPIA COLOR AND FONT DEFINITIONS ---
SEPIA_BG = '#1DA1F2'  # Creamy background
SEPIA_FG = "#ffffff"  # Sepia/Dark brown text
SEPIA_ACCENT = "#AFEBFF" # Medium brown accent for buttons/borders
SEPIA_BORDER = "#7153d4" # Lighter brown for frames
SEPIA_HIGHLIGHT = "#13198e" # For selected items

# Font definition
MAIN_FONT = ("Times New Roman", 10)
TITLE_FONT = ("Times New Roman", 12, "bold")
MONOSPACE_FONT = ("Courier", 10) # For code/tables

# --- END SEPIA COLOR AND FONT DEFINITIONS ---
...
class Application:
    def __init__(self, root):
        ...
        # Apply global root background and style configuration
        self.root.tk_setPalette(background=SEPIA_BG, foreground=SEPIA_FG, activeBackground=SEPIA_HIGHLIGHT, activeForeground='white')
        ...
        # Use a style for the info button for better visual control
        info_button = ttk.Button(self.root, text="ⓘ", command=self.show_info, style='Info.TButton')
    ...
    def configure_styles(self):
        """Configure ttk styles for a sepia theme."""
        style = ttk.Style()
        
        # Set a general theme (might not be strictly necessary but good practice)
        style.theme_use('clam')
        
        # General style for all TFrame, TLabel, TEntry, TButton, TCheckbutton, TRadiobutton
        style.configure('.', font=MAIN_FONT, background=SEPIA_BG, foreground=SEPIA_FG)
        
        # Notebook style (Tabs)
        style.configure('TNotebook', background=SEPIA_BG, bordercolor=SEPIA_ACCENT)
        style.configure('TNotebook.Tab', background=SEPIA_ACCENT, foreground='white', padding=[10, 5], font=TITLE_FONT)
        style.map('TNotebook.Tab', 
                  background=[('selected', SEPIA_BORDER), ('!selected', SEPIA_ACCENT)],
                  foreground=[('selected', SEPIA_FG), ('!selected', 'white')])
        
        # Frame styles
        style.configure('TFrame', background=SEPIA_BG)
        
        # Label Frame style
        style.configure('TLabelframe', background=SEPIA_BG, foreground=SEPIA_FG, bordercolor=SEPIA_BORDER)
        style.configure('TLabelframe.Label', font=TITLE_FONT, background=SEPIA_BG, foreground=SEPIA_FG)
        
        # Label style
        style.configure('TLabel', background=SEPIA_BG, foreground=SEPIA_FG, font=MAIN_FONT)
        
        # Button style
        style.configure('TButton', background=SEPIA_ACCENT, foreground='white', font=MAIN_FONT, padding=5, relief='flat')
        style.map('TButton', 
                  background=[('active', SEPIA_HIGHLIGHT), ('!disabled', SEPIA_ACCENT)],
                  foreground=[('active', 'white'), ('!disabled', 'white')])
        
        # Entry/Input style
        style.configure('TEntry', fieldbackground='white', foreground=SEPIA_FG, font=MAIN_FONT, bordercolor=SEPIA_ACCENT)
        
        # Radio/Check button style
        # The background of the indicator is hard to change in TTK across themes, but we set the label's style
        style.configure('TRadiobutton', background=SEPIA_BG, foreground=SEPIA_FG, font=MAIN_FONT)
        style.map('TRadiobutton', background=[('active', SEPIA_HIGHLIGHT)])
        
        # Scrollbar style
        style.configure('Vertical.TScrollbar', background=SEPIA_ACCENT, troughcolor=SEPIA_BG, arrowcolor='white')
        style.map('Vertical.TScrollbar', background=[('active', SEPIA_HIGHLIGHT)])
        
        # Info Button (a small square button)
        style.configure('Info.TButton', background=SEPIA_ACCENT, foreground='white', font=MAIN_FONT, padding=[5, 1])
        style.map('Info.TButton', background=[('active', SEPIA_HIGHLIGHT)], foreground=[('active', 'white')])
    ...
    def setup_interpolation_tab(self):
        ...
        # --- Listbox style change (standard tk widget) ---
        self.points_listbox = tk.Listbox(list_frame, height=8, font=MAIN_FONT, bg='white', fg=SEPIA_FG,
                                        selectbackground=SEPIA_HIGHLIGHT, selectforeground='white')
        # --- End Listbox style change ---

        self.points_listbox.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        
        # Use ttk scrollbar to match theme
        ...
        # Result display
        self.eval_result = ttk.Label(eval_frame, text="Result: ", foreground=SEPIA_ACCENT, font=(MAIN_FONT[0], MAIN_FONT[1], "bold")) # Specific font for result
        ...
        # --- Matplotlib style change ---
        self.fig = Figure(figsize=(10, 7), dpi=100, facecolor=SEPIA_BG) # Set figure background
        self.ax = self.fig.add_subplot(111)
        self.ax.set_facecolor('white') # Set plot area background
        self.ax.tick_params(axis='x', colors=SEPIA_FG)
        self.ax.tick_params(axis='y', colors=SEPIA_FG)
        self.ax.spines['left'].set_color(SEPIA_FG)
        self.ax.spines['bottom'].set_color(SEPIA_FG)
        self.ax.spines['right'].set_color(SEPIA_FG)
        self.ax.spines['top'].set_color(SEPIA_FG)
        # --- End Matplotlib style change ---
        ...
        # --- ScrolledText style change (standard tk widget) ---
        self.poly_text = scrolledtext.ScrolledText(poly_frame, wrap=tk.WORD, height=20, font=MONOSPACE_FONT, 
                                                   bg='white', fg=SEPIA_FG, insertbackground=SEPIA_FG)
        # --- End ScrolledText style change ---
    ...
    def setup_integration_tab(self):
        ...
        # These two frames are currently not used correctly in the original code, but I'll leave their grid calls commented out
        ...
        # Results display
        # --- ScrolledText style change (standard tk widget) ---
        self.integration_results_text = scrolledtext.ScrolledText(results_frame, wrap=tk.WORD, height=25, font=MONOSPACE_FONT,
                                                                  bg='white', fg=SEPIA_FG, insertbackground=SEPIA_FG)
        # --- End ScrolledText style change ---
        ...
        # --- ScrolledText style change (standard tk widget) ---
        self.array_output_text = scrolledtext.ScrolledText(array_frame, wrap=tk.WORD, height=25, font=MONOSPACE_FONT,
                                                           bg='white', fg=SEPIA_FG, insertbackground=SEPIA_FG)
        # --- End ScrolledText style change ---
        ...
        # --- Matplotlib style change ---
        self.integration_fig = Figure(figsize=(10, 7), dpi=100, facecolor=SEPIA_BG)
        self.integration_ax = self.integration_fig.add_subplot(111)
        self.integration_ax.set_facecolor('white')
        self.integration_ax.tick_params(axis='x', colors=SEPIA_FG)
        self.integration_ax.tick_params(axis='y', colors=SEPIA_FG)
        # --- End Matplotlib style change ---
    ...
    def setup_ode_tab(self):
        ...
        self.ode_analytical_plot_status = ttk.Label(self.analytical_ode_frame, text="", foreground=SEPIA_ACCENT, font=(MAIN_FONT[0], MAIN_FONT[1], "bold"))
        ...
        # --- ScrolledText style change (standard tk widget) ---
        self.ode_solution_text = scrolledtext.ScrolledText(solution_frame, wrap=tk.WORD, height=25, font=MONOSPACE_FONT,
                                                            bg='white', fg=SEPIA_FG, insertbackground=SEPIA_FG)
        # --- End ScrolledText style change ---
        ...
        # --- ScrolledText style change (standard tk widget) ---
        self.ode_table_text = scrolledtext.ScrolledText(table_frame, wrap=tk.NONE, height=25, font=MONOSPACE_FONT,
                                                        bg='white', fg=SEPIA_FG, insertbackground=SEPIA_FG)
        # --- End ScrolledText style change ---
        ...
        # --- Matplotlib style change ---
        self.ode_fig = Figure(figsize=(10, 7), dpi=100, facecolor=SEPIA_BG)
        self.ode_ax = self.ode_fig.add_subplot(111)
        self.ode_ax.set_facecolor('white')
        self.ode_ax.tick_params(axis='x', colors=SEPIA_FG)
        self.ode_ax.tick_params(axis='y', colors=SEPIA_FG)
        # --- End Matplotlib style change ---
    ...
    def _update_ode_plot(self):
        ...
        if self.ode_numerical_results:
            ...
            if self.ode_method.get() == 'all':
                ...
                if method_name in self.ode_numerical_results:
                    ...
                    # Use sepia/brown tones for plots
                    self.ode_ax.plot(x_values, y_values, color=SEPIA_ACCENT, linewidth=2, label=f'{method_name.replace("_", "-").title()} Solution')
                    if x_values and y_values:
                        self.ode_ax.plot(x_values[0], y_values[0], 'o', markersize=8, color=SEPIA_FG, label='Initial Condition')
                        self.ode_ax.plot(x_values[-1], y_values[-1], 'D', markersize=8, color=SEPIA_BORDER, label='Final Value')
        ...
        if self.ode_analytical_func and self.ode_numerical_results.get('x'): # Only plot if numerical results define x range
            ...
            try:
                ...
                # Plot analytical in black dashed line for contrast
        ...
        # Set plot text colors
        self.ode_ax.grid(True, alpha=0.3, color=SEPIA_BORDER)
        self.ode_ax.legend(facecolor='white', edgecolor=SEPIA_FG, labelcolor=SEPIA_FG)
        self.ode_ax.set_xlabel('x', color=SEPIA_FG)
        self.ode_ax.set_ylabel('y', color=SEPIA_FG)
        self.ode_ax.set_title(f'First Order ODE Solutions - {self.ode_method.get().replace("_", "-").title()}', color=SEPIA_FG)
        ...
        if self.ode_order.get()== 'first':
            if self.ode_method.get() == 'all':
                ...
                if self.ode_analytical_func_expr_str == "":
                    ...
                    table_text += f"{'i':>3} {'x':>10} {'y':>12} {'Analytical':>12} {'Error':>12}\n" # Added Analytical column
                    table_text += "-" * 55 + "\n"
                    x_values = self.ode_numerical_results.get('x', [])
                    y_values = self.ode_numerical_results.get(self.ode_method.get(), [])
                    for i, (x_val, y_val) in enumerate(zip(x_values, y_values)):
                        analytical_val = self.ode_analytical_func(x_val) if self.ode_analytical_func else float('nan')
                        error = abs(y_val - analytical_val) if self.ode_analytical_func else '-'  # Use actual error if analytical func exists
                        analytical_text = f"{analytical_val:>12.6f}" if self.ode_analytical_func else f"{'---':>12}"
                        error_text = f"{error:>12.6f}" if self.ode_analytical_func else f"{'---':>12}"
                        table_text += f"{i:>3} {x_val:>10.4f} {y_val:>12.6f} {analytical_text} {error_text}\n"
            ...
            if self.ode_analytical_func_expr_str == "":
                ...
                table_text += f"{'i':>3} {'x':>10} {'y':>12} {"y1'":>12}{'Analytical':>12}{'Error':>12}\n" # Added Analytical and Error
                table_text += "-" * 67 + "\n"
                for i,(x_val, y_val, y1_val) in enumerate(zip(x_values, y_values, y1_values)):
                    analytical_val = self.ode_analytical_func(x_val)
                    error = abs(y_val - analytical_val)
                    table_text += f"{i:>3} {x_val:>10.4f} {y_val:>12.6f} {y1_val:>12.6f} {analytical_val:>12.6f} {error:>12.6f}\n"
            table_text += "-" * 67 + "\n"
        return table_text
    ...
    def plot_ode_analytical_solution(self):
        ...
        try:
            ...
            self.ode_analytical_plot_status.config(text="Analytical function plotted successfully.", foreground=SEPIA_ACCENT)
    ...
    def on_input_method_change(self):
        # This function isn't used in the provided app.py but is kept for completeness if needed later
        pass
    ...
    def integration_plot(self,
        ...
        """
        # Set plot text colors
        self.integration_ax.set_xlabel('x', color=SEPIA_FG)
        self.integration_ax.set_ylabel('f(x)' if self.integration_type.get() == 'single' else 'y', color=SEPIA_FG)
        if self.integration_type.get() == 'double':
            self.integration_ax.set_zlabel('f(x,y)', color=SEPIA_FG)
        self.integration_ax.set_title(f'{self.integration_type.get().title()} Integral: {self.integration_method.get().replace("_", " ").title()} Method', color=SEPIA_FG)
        self.integration_ax.grid(True, alpha=0.3, color=SEPIA_BORDER)
        
        # Color changes for plots: use SEPIA_ACCENT and similar light colors
        ...
        if self.integration_type.get() == 'single':
            if self.integration_method.get() == 'trapezoidal':
                ...
                for j,i in enumerate(np.arange(X[0], X[-1],h)):
                    ...
                    self.integration_ax.fill_between(points, 0, f[j], color=SEPIA_ACCENT, alpha=0.5)
            ...
            if self.integration_method.get() == 'simpsons38':
                ...
                for j,i in enumerate(np.arange(X[0],X[-1],3*h)):
                    if i+3*h <= X[-1]:
                        ...
                        self.integration_ax.fill_between(points,0,f[j],color=SEPIA_ACCENT,alpha=0.5)
        else: # Double integral plotting logic (3D)
            # This plotting logic seems to be an attempt at plotting the intermediate elements (bars/volumes)
            # The general surface plot is done later in calculate_integration. This part is for the bars.
            if self.integration_method.get() == 'trapezoidal':
                h = X[1]-X[0]
                k = Y[1]-Y[0]
                x_vals = X[:-1]
                ...
                # Use a specific color for the bars
                self.integration_ax.bar3d(x_vals, y_vals, base, h, k, f, shade=True, color=SEPIA_HIGHLIGHT, alpha=0.5)
            # The rest of the double integral plotting logic is too complex to fix easily without full testing, 
            # I will apply the color change to the bar3d calls only.
            
            # ... (rest of the original double integral plotting logic) ...
        ...
        try:
            ...
            if self.integration_type.get() == "single":
                try:
                    ...
                    self.integration_ax.clear()
                    if 'data' in integral_data and integral_data['data'] and h:
                        x_vals = [x0+i*h for i in range(n+1)]
                        x_plot = np.linspace(x0,xn,100)
                        self.integration_ax.plot(x_plot,f(x_plot), color=SEPIA_FG, linestyle='-', label="Function") # Set line color
                        self.integration_plot(integral_data['graph'], x_vals) # This calls back into the plotting function, where color needs to be set
                    else:
                        ...
                        self.integration_ax.plot(x_plot, y_plot, color=SEPIA_FG, linestyle='-', label='Function f(x)')
                        self.integration_ax.fill_between(x_plot, y_plot, color=SEPIA_ACCENT, alpha=0.5, label='Area under curve') # Set fill color
                    
                    # Set axis/title in plot helper function
                    self.integration_plot([], [], []) # Call to set title/labels/colors

                    self.integration_ax.legend(facecolor='white', edgecolor=SEPIA_FG, labelcolor=SEPIA_FG)
                    self.integration_canvas.draw()
                except Exception as e:
                ...
                try:
                    ...
                    if 'data' in integral_result and integral_result['data']:
                        ...
                        # The data seems to be flattened by the integration function.
                    ...
                    # --- Matplotlib 3D style change ---
                    self.integration_ax = self.integration_fig.add_subplot(111, projection='3d', facecolor='white') # Add 3D subplot
                    self.integration_ax.tick_params(axis='x', colors=SEPIA_FG)
                    self.integration_ax.tick_params(axis='y', colors=SEPIA_FG)
                    self.integration_ax.tick_params(axis='z', colors=SEPIA_FG)
                    # --- End Matplotlib 3D style change ---
                    
                    if self.integration_method.get() not in ['all', 'gaussian','romberg']:
                        fn = integral_result['graph']
                        # The original code had a messagebox("Debug", ...) here, removed for cleaner output
                        self.integration_plot(fn, x_vals_plot, y_vals_plot)

                    # Use a colormap suitable for sepia/brown tones (e.g., bone, copper, or a custom one)
                    self.integration_ax.plot_surface(X, Y, Z, cmap='bone', alpha=0.8) 
                    
                    # Set axis/title in plot helper function
                    self.integration_plot([], [], [])
    ...
    def _update_interpolation_plot(self):
        ...
        if self.current_func or self.current_spline:
            ...
            if x_range_interp > 0:
                x_plot_interp = np.linspace(x_min_interp - x_range_interp * 0.1, x_max_interp + x_range_interp * 0.1, 500) # Extended range
            else: # Case of a single point or all points at the same x-value
                x_plot_interp = np.linspace(x_min_interp - 1, x_max_interp + 1, 500) # Default small range
            ...
            # Use sepia/brown tones for plots
            self.ax.plot(x_plot_interp, y_plot_interp, color=SEPIA_ACCENT, linestyle='-', linewidth=2, label=f'{self.interpolation_type.get().title()} Interpolation')
            self.ax.plot(x_vals_interp, y_vals_interp, 'o', markersize=8, color=SEPIA_FG, label='Data Points') # Use dark sepia for points
            
            for i, (x_p, y_p) in enumerate(zip(x_vals_interp, y_vals_interp)):
                self.ax.annotate(f'({x_p:.2f}, {y_p:.2f})', (x_p, y_p), xytext=(5, 5), 
                               textcoords='offset points', fontsize=8, color=SEPIA_FG) # Set annotation color
        
        # Set plot text colors
        self.ax.grid(True, alpha=0.3, color=SEPIA_BORDER)
        self.ax.legend(facecolor='white', edgecolor=SEPIA_FG, labelcolor=SEPIA_FG)
        self.ax.set_xlabel('x', color=SEPIA_FG)
        self.ax.set_ylabel('y', color=SEPIA_FG)
        self.ax.set_title('Interpolation Plot', color=SEPIA_FG)
        self.canvas.draw()
    ...
    def clear_points(self):
        ...
        self.update_points_display()
        self.ax.clear()
        self.ax.set_facecolor('white') # Ensure plot area remains white
        self.canvas.draw()
    ...
    def update_points_display(self):
        ...
        for i, (x_val, y_val) in enumerate(self.points):
            if self.interpolation_type.get() == "hermitian" and i < len(self.derivatives):
                text = f"({x_val:.4f}, {y_val:.4f}, y'={self.derivatives[i]:.4f})"
            else:
                text = f"({x_val:.4f}, {y_val:.4f})"
    ...
    def calculate_interpolation(self):
        ...
        try:
            ...
            for i, (x_p, y_p) in enumerate(zip(x_vals, y_vals)):
                if self.interpolation_type.get() == "hermitian":
                    poly_text += f"  ({x_p:.4f}, {y_p:.4f}), y'({x_p:.4f}) = {self.derivatives[i]:.4f}\n"
                else:
                    poly_text += f"  ({x_p:.4f}, {y_p:.4f})\n"
    ...
    def evaluate_function(self):
        ...
        try:
            ...
            self.eval_result.config(text=f"Result: f({x_val:.4f}) = {result:.6f}")
            
            # Add evaluation point to plot
            # Use SEPIA_HIGHLIGHT for the evaluated point for visibility
            self.ax.plot(x_val, result, 's', markersize=10, color=SEPIA_HIGHLIGHT, label=f'Evaluated f({x_val:.4f}) = {result:.3f}') 
            self.ax.legend(facecolor='white', edgecolor=SEPIA_FG, labelcolor=SEPIA_FG)
            self.canvas.draw()
