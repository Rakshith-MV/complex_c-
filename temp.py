...
class Application:
    def __init__(self, root):
        self.root = root
        self.root.title("Interpolation, Integration & ODE Calculator")
        ...
        self.ode_method = tk.StringVar(value="picard")
    ...
    def setup_integration_tab(self):
        ...
        ttk.Label(single_grid, text="xâ‚€:").grid(row=0, column=0, padx=(0, 5), sticky=tk.W)
        self.x0_entry = ttk.Entry(single_grid, width=10)
        self.x0_entry.grid(row=0, column=1, padx=(0, 15))
        
        ttk.Label(single_grid, text="xâ‚™:").grid(row=0, column=2, padx=(0, 5), sticky=tk.W)
        ...
        # First row
        ttk.Label(double_grid, text="xâ‚€:").grid(row=0, column=0, padx=(0, 5), sticky=tk.W)
        self.x0_double_entry = ttk.Entry(double_grid, width=8)
        self.x0_double_entry.grid(row=0, column=1, padx=(0, 10))
        
        ttk.Label(double_grid, text="xâ‚™:").grid(row=0, column=2, padx=(0, 5), sticky=tk.W)
        ...
        # Second row
        ttk.Label(double_grid, text="yâ‚€:").grid(row=1, column=0, padx=(0, 5), sticky=tk.W, pady=(5, 0))
        self.y0_entry = ttk.Entry(double_grid, width=8)
        self.y0_entry.grid(row=1, column=1, padx=(0, 10), pady=(5, 0))
        
        ttk.Label(double_grid, text="yâ‚™:").grid(row=1, column=2, padx=(0, 5), sticky=tk.W, pady=(5, 0))
    ...
    def setup_ode_tab(self):
        ...
        ttk.Radiobutton(methods_row2, text="Shooting Method", variable=self.ode_method, 
                       value="shooting").pack(side=tk.LEFT)
        ...
        ttk.Label(params_frame, text="n (steps):").grid(row=1, column=0, padx=(0, 5), sticky=tk.W, pady=(5, 0))
        ...
        ttk.Label(params2_frame, text="n (steps):").grid(row=1, column=2, padx=(0, 5), sticky=tk.W, pady=(5, 0))
        self.ode2_n_entry = ttk.Entry(params2_frame, width=8)
        self.ode2_n_entry.grid(row=1, column=3, padx=(0, 10), pady=(5, 0))
    ...
    def clear_ode(self):
        ...
        self.ode2_n_entry.delete(0, tk.END)
    ...
    def solve_ode(self):
        ...
        try:
            if self.ode_order.get() == "first":
                ...
                solution_text += f"Step size: h = {h}, Number of steps: n = {n}\n\n"
                
                # Placeholder solution (you would implement actual ODE solving here)
                x_values = [x0 + i*h for i in range(n+1)]
                y_values = [y0 + i*0.1 for i in range(n+1)]  # Placeholder values
            ...
            for i, (x, y) in enumerate(zip(x_values, y_values)):
                ...
                table_text += f"{i:>3} {x:>10.4f} {y:>12.6f} {error:>12.6f}\n"
            ...
            # Plot solution
            self.ode_ax.clear()
            self.ode_ax.plot(x_values, y_values, 'b-', linewidth=2, label=f'{self.ode_method.get().replace("_", "-").title()} Solution')
            ...
            self.ode_ax.set_title(f'{self.ode_order.get().title()} Order ODE Solution - {self.ode_method.get().replace("_", "-").title()} Method')
            ...
            if self.integration_type.get() == "single":
                try:
                    ...
                    if(self.integration_method.get() != 'romberg'):
                        ...
                        results_text += f"Limits: xâ‚€ = {x0}, xâ‚™ = {xn}\n"    
                        ...
                        messagebox.showinfo("Info", "Romberg method isn't active yet")
                        return  
                except:
                ...
                try:
                    ...
                    #once all is established, remove here!!!
                    if(self.integration_method.get() not in  ['romberg', 'gaussian','all','simpsons38']):
                        ...
                        messagebox.showinfo("Info", "Gaussian method isn't active yet for double integral")
                        return
                    elif(self.integration_method.get() == 'romberg'):
                        messagebox.showinfo("Info", "Romberg method isn't active yet for double integral")
                        return
                    ...
                    results_text += f"X limits: xâ‚€ = {x0}, xâ‚™ = {xn}, h = {h}\n"
                    results_text += f"Y limits: yâ‚€ = {y0}, yâ‚™ = {yn}, k = {k}\n\n"
